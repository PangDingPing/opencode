...
// 重置密码弹窗：admin 为指定用户重置密码
const AdminResetPasswordDialog: Component<Props> = (props) => {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      const res = await fetch(`/api/admin/users/${props.userId}/reset-password`, {
        method: "POST",
      ...
      showToast({ title: "密码已重置", description: `用户 ${props.username} 下次登录需改密`, variant: "success" })
      dialog.close()
      props.onDone()
    } catch {
      setError("网络错误，请重试")
